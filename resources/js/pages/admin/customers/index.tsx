...
export default function AdminCustomersIndexPage({ customers: customerPage }: { customers: { data: Customer[] } }) {
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Subscribers and their workspace links</p>
        </div>
      </div>
      ...
      <Card className="overflow-hidden rounded-xl border bg-card shadow-sm">
        <CardHeader className="border-b bg-muted/30 py-4">
          ...
          <CardDescription>Select a row to open the customer profile</CardDescription>
        </CardHeader>
                ...
                <TableRow
                  ...
                  onKeyDown={(event) => {
                    if (event.key === 'Enter' || event.key === ' ') {
                      ...
                      router.visit(customerShow.url(customer.id));
                    }
                  }}
                >
                  ...
                  <TableCell>{customer.plan?.name ?? 'N/A'}</TableCell>
                  <TableCell>
